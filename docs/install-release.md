...
After all four trusted version statuses are green, the coordinator attaches the macOS and Linux assets, invokes publication for that exact tag without an interactive selector, and follows the established immutable GitHub release, direct installer, Homebrew, optional APT, and WinGet distribution flow. A failed phase stops later phases and terminates the coordinator's local verifier processes and SSH client. The remote Windows process can continue after its SSH client is terminated, so confirm that machine is idle before retrying a failed cross-machine wave. Rerun the same command after correcting the failure: an exactly tagged current `Release vX.Y.Z` commit resumes its existing draft or incomplete immutable distribution instead of incrementing again. The required increment argument is ignored for that tagged resume path. A matching release commit without its tag first runs the ordinary trusted checks, then lets the lower-level release command repair the missing tag and draft.
