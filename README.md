...
If any target object fails, neither repository config location is changed and a retry resumes from the durable receipt. If a later repository-config write itself fails, the uploaded objects and receipt remain safe; rerun the same command, then verify both `.lfsconfig` and `git config --local --get lfs.url` before changing or removing the source endpoint. Do not purge source LFS data until the resumed run revalidates the receipt and reports complete.
