...
- **[cli] Init route base URL**: `init --server` rejects trailing-slash server bases and appends the Git remote-derived LFS route directly, matching `server.public_url` behavior while preserving optional proxy base paths. See `LfsInitRoute` in src/init.rs.
