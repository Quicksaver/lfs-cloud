...
- **[build] Fleet release resumption**: Fast-forward only a clean Windows `main` on `E:\`, target Windows and publication by exact tag, and skip ordinary base checks when `HEAD` is already a `Release vX.Y.Z` commit so a partial draft cannot gain a Windows success status without its asset upload. See `release_all_prepare_candidate` in scripts/release-all.sh.
