...
- **[auth] GitHub PAT boundary**: GitHub authentication is single-account PAT-only; require the caller to present the exact configured PAT over protected transport, then issue only a local LFS token so Git credentials never store the upstream PAT. See `GitHubPersonalAccessTokenLoginRouteState` in src/github_auth.rs.
- **[auth] Local LFS sessions**: Git LFS credentials are separate LFS Cloud bearer tokens backed by local session metadata; never store or hand the configured GitHub PAT to Git credential-helper paths. See `LocalLfsSessionStore` in src/sessions.rs.
...
- **[auth] Session-key migrations**: When an authentication mode changes the durable-session root secret, invalidate older protected rows in a schema migration before loading sessions so undecryptable legacy credentials cannot block startup. See `PAT_AUTHENTICATION_SESSION_MIGRATION` in src/metadata.rs.
...
- **[auth] Batch authorization token boundary**: Local LFS sessions retain the configured GitHub PAT only as private server-side state so batch requests can re-check repository permissions while Git LFS receives only the local LFS Cloud token. See `LfsSessionRecord` in src/sessions.rs.
- **[storage] gcloud ADC isolation**: `gcloud` must set an isolated `CLOUDSDK_CONFIG`, suppress prompts and ambient credential overrides, and require generated `application_default_credentials.json`; explicit login scopes need both `cloud-platform` and `drive.file`. See `GoogleDriveGcloudTokenProvider` in src/google_drive.rs.
- **[compatibility] Windows gcloud launcher**: Default to `gcloud.cmd` on Windows because a bare `gcloud` process name does not resolve the standard Cloud SDK command launcher there; Node smoke probes must invoke the batch launcher through `cmd.exe`. See `DEFAULT_GCLOUD_EXECUTABLE` in src/server_config.rs and `gcloudInvocation` in .agents/skills/smoke-test/scripts/smoke-test.ts.
...
- **[storage] Drive startup readiness**: Validate every configured gcloud ADC source and writable root before binding the server listener, and reuse the minted token cache so readiness does not cause an immediate second CLI invocation. See `GoogleDriveTransferStore::validate_storage_providers` in src/server.rs.
...
- **[storage] Drive indexed sharding**: Server lookups verify SQLite backend IDs with direct `files.get`; stale IDs fall back to root/shard discovery and conditional metadata repair. New uploads use deterministic first-byte shard folders while legacy root objects remain discoverable. See `GoogleDriveTransferStore::lookup_and_repair_object` in src/server.rs.
...
- **[providers] Config registration boundary**: Concrete provider config variants register runtime adapters, readiness checks, and metadata descriptors in one factory module; keep provider-neutral callers on those methods, while the Drive-specific server transfer path remains separately scoped. See `RepositoryProviderConfig::build_provider` in src/provider_factory.rs.
...
- **[storage] Drive provider layers**: `GoogleDriveObjectStore` supplies direct object operations; configured migration uploads use `GoogleDriveStorageProvider` for namespace pinning and metadata-backed locking, while server LFS transfers separately record verified-object metadata. See `GoogleDriveStorageProvider` in src/provider_factory.rs.
...
- **[metadata] Config route reconciliation**: Config sync tombstones routes for removed or renamed mappings and marks them inactive before current upserts, preserving object/attempt history while releasing unique public routes. See `release_inactive_repository_routes` in src/metadata.rs.
- **[metadata] Verified object upsert**: Successful uploads upsert one row per repo/storage/OID/size and refresh stale backend IDs in place while preserving the first creator's attribution, so retry and repair paths neither create duplicates nor rewrite provenance. See `record_verified_object` in src/metadata.rs.
- **[metadata] Transfer attempt lifecycle**: Start durable attempt rows only after route, session, size, and permission validation; complete them with backend IDs on success or fixed secret-free diagnostics on failure, leaving `started` only for interrupted work. See `MetadataDatabase::start_transfer_attempt` in src/metadata.rs.
...
- **[auth] Config debug secrecy**: `ServerConfig` debug output can cross logs and test failures, so provider configs that retain resolved PATs must redact custom `Debug` fields instead of deriving them. See `GitHubProviderConfig` in src/server_config.rs.
...
- **[auth] GitHub identity casing**: Match GitHub host/owner/repository identities without ASCII case sensitivity while preserving their original spelling for display and provider calls; keep the Git LFS protocol suffix case-sensitive. See `ServerConfig::repository_mapping_for_identity` in src/server_config.rs.
...
- **[auth] Durable session key**: Production sessions persist only local-token hashes and AEAD-protected GitHub PATs; the encryption key derives from the configured PAT, which must stay stable until active sessions expire or are removed. See `production_session_store` in src/server.rs.
...
- **[migration] Upload checkpoints**: Synchronize each completed object before reporting it, resume only durable completions, and retain failed objects as structured retry outcomes so one provider failure does not discard independent progress. See `upload_migration_objects_to_storage_with_options` in src/migration.rs.
...
- **[migration] Purge receipt boundary**: A dry-run inventory proves only planned scope, not destination upload; emit purge input only from a durable completion receipt that integrity-verifies every object. See `write_migration_source_purge_report` in src/cli.rs.
- **[migration] Execution reconfiguration boundary**: Refresh source refs and require all-ref inventory, then fetch, checkpoint, and verify every object before changing either Git config location; a partial target result must leave source routing intact. See `run_migrate_execution_from_dir` in src/cli.rs.
- **[migration] Historical endpoint persistence**: A newly committed `.lfsconfig` disappears when checking out older commits, so migration also writes repository-local `lfs.url`; fresh clones need `init --local` before visiting pre-config history. See `execute_migration_with_storage` in src/cli.rs.
...
- **[metadata] Async SQLite boundary**: Keep startup metadata work synchronous, but dispatch request-path writes through Tokio's blocking pool so SQLite busy waits and the connection mutex cannot block async workers. See `MetadataDatabase::record_verified_object_async` in src/metadata.rs.
