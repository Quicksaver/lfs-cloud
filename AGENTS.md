...
- **[auth] Git credential path scope**: Approve LFS Cloud tokens with `credential.useHttpPath=true`; otherwise Git's credential machinery can store only the host and reuse one token across multiple repo-specific LFS URL paths. See `GitCredentialApproval` in src/credentials.rs.
