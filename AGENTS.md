...
- **[config] Setup environment interpolation**: Resolve persisted provider URLs and credential paths through the server configuration interpolator before CLI setup uses them, while preserving raw references for YAML writes. See `github_provider_host_with_env` and `resolve_setup_path_with_env` in src/cli/configuration.rs.
