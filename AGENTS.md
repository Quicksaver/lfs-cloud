...
- **[server] Download batch placeholder**: Download batch responses are Git LFS JSON already, but object availability intentionally returns per-object `501` errors until metadata/storage lookup is wired. See `download_batch_response_pending_storage_lookup` in src/server.rs.
- **[server] Upload batch placeholder**: Upload batch responses use the same Git LFS JSON object-level error shape as downloads until object availability lookup and upload transfer handling are wired. See `upload_batch_response_pending_storage_lookup` in src/server.rs.
