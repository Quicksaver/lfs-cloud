...
- **[tests] Storage contract parity**: Run the shared storage lifecycle contract against integration fakes, direct provider adapters, and migration wrappers; repository-bound providers may reject foreign namespaces while multi-namespace providers must isolate them. See `assert_storage_provider_contract` in tests/support/storage_provider_contract.rs.
