...
- **[cli] Remote credential safety**: Git remote parsing rejects credentialed HTTPS URLs and query strings before deriving route identity, so future `init` code cannot accidentally echo tokens while building LFS endpoint paths. See `GitRemote` in src/git.rs.
