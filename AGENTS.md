...
- **[tests] Local fake E2E**: The local end-to-end coverage uses fake repository/storage providers plus the shared cache to verify init, authorization, upload/download, and checkout hydration without requiring real GitHub, Drive, or git-lfs binaries. See `local_init_upload_download_and_checkout_flow_uses_fake_providers` in tests/local_end_to_end.rs.
