...
- **[cache] CLI registry refresh**: CLI `hydrate`, `dehydrate`, and `gc` refresh the current Git worktree registration before cache mutation or collection; GC safety depends on keeping this command boundary in place. See `register_current_worktree` in src/cli.rs.
