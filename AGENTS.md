...
- **[config] Installed config default**: Omitted `--config` resolves `${HOME}/.config/lfscloud/config.yml` or `%APPDATA%\lfscloud\config.yml`, with the Windows roaming profile as fallback; never make installed CLI behavior depend on the process working directory. See `ServerConfig::default_path` in src/server_config/config.rs.
