...
- **[tests] Live provider composition**: Seed a durable local session, then call the public `serve` path so gated provider tests cover production GitHub authorization, Drive transfers, SQLite recording, and action URLs without depending on an interactive browser callback. See `live_server_upload_download_records_drive_and_sqlite_state` in tests/external_integrations.rs.
