...
- **[build] Windows SSH Rust proxies**: Key-authenticated Windows OpenSSH can reject rustup-created executable symlinks, and PowerShell 7.6 can reject native file redirection. Resolve reparse-point `cargo`/`rustc` commands through the active rustup toolchain and stream verifier output through the job channel. See `Resolve-ReleaseNativeInvocation` in scripts/lib/release-common.ps1 and `ui_run_with_live_stdout` in scripts/lib/terminal-ui.ps1.
