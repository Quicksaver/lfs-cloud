...
- **[auth] Credential helper preflight**: Check `git config --get-all credential.helper` before `git credential approve`; Git can accept an approve request with no helper configured and then persist nothing. See `GitCredentialApproval` in src/credentials.rs.
