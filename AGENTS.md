...
- **[storage] Default Drive root identity**: Default setup resolves the app-marked `.lfscloud` folder by name but persists its real Drive ID; never pass the display name to runtime storage APIs as an ID. See `GoogleDriveRootProvisioner` in src/google_drive/root.rs.
