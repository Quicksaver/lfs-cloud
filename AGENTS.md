...
- **[providers] Config registration boundary**: Concrete config variants register runtime adapters, readiness, metadata descriptors, and optional server capabilities in one factory module; production callers stay on provider-neutral registries. See `ServerStorageProviderFactory` in src/provider_factory.rs.
...
- **[storage] Drive provider layers**: `GoogleDriveStorageProvider` supplies the shared contract; migration acquires its durable upload lock, while the server owns that lock and adds generic metadata repair around the same provider plus optional indexed-lookup and streaming capabilities. See `StorageProviderTransferStore` in src/server.rs.
