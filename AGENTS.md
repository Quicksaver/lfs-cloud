...
- **[errors] CLI diagnostics**: Default command failures render every retained typed cause plus an automatically captured backtrace; `--quiet` changes only presentation, while secret-bearing credential streams remain unavailable to both modes. See `write_error_report` in src/main.rs.
...
- **[tests] Windows Node PATH overrides**: Remove case-variant inherited PATH keys before spawning fake tools; Windows treats environment names case-insensitively, so duplicate `Path` and `PATH` entries can bypass the requested fixture directory. See `runCommand` in .agents/skills/smoke-test/scripts/smoke-test.ts.
- **[tests] Windows command shims**: Rust's bare executable lookup does not select `.cmd` smoke shims from `PATH`; pass the shim's explicit path through the supported executable override. See `resolve_github_repository_id` in src/cli/configuration.rs.
...
- **[compatibility] Repository dot segments**: GitHub repository names may contain consecutive periods; reject only complete `.` or `..` path components, and keep lookup and persisted-route validation aligned. See `validate_route_component` in src/server_config/validation.rs.
...
- **[config] Installed config default**: Omitted `--config` resolves below the platform's per-user config directory; every file-based load initializes a missing path with private Unix modes. Keep installed CLI behavior independent of the working directory. See `ServerConfig::ensure_file_exists` in src/server_config/config.rs.
- **[config] Interactive provider setup**: Keep terminal arrow menus paired with a numbered non-terminal fallback so configuration flows remain usable by humans and deterministic smoke fixtures; repository mappings infer IDs only after selecting configured providers. See `prompt_select_index` in src/cli/configuration.rs.
- **[config] Setup path resolution**: Resolve persisted provider URLs and credential paths through the server configuration resolver before CLI setup uses them, including current-user `~/` paths, while preserving raw references for YAML writes. See `github_provider_host_with_env` in src/cli/configuration.rs and `resolve_config_directory` in src/server_config/resolution.rs.
- **[cli] Repository server inference**: Client commands accept an inferred server only when local `lfs.url` or committed `.lfsconfig` exactly reconstructs the current remote's LFS Cloud route; explicit `--server` wins, and initial legacy migration still needs it. See `resolve_repository_lfs_route` in src/cli/mod.rs.
...
- **[auth] Credential lookup diagnostics**: Suppress `git credential fill` stderr because a failing helper can echo a stored password before lookup output reveals which value must be redacted; wrap that retained cause with the target URL and matching `lfscloud login` recovery. See `GitCredentialLookup::lookup_with_git_program` in src/credentials.rs.
...
- **[migration] Target address aliases**: When explicit `--server` selects another origin for the same repository route, exclude both that route and a repository-shaped configured `lfs.url`; origin-only comparison can misclassify a Tailscale target as the legacy source when loopback is selected. See `migration_target_endpoints` in src/cli/migration.rs.
- **[migration] Upload progress**: Flush one start line per target-missing server upload; the next start or final report implies success, while a failure follows the last attempted object. Target-present objects remain final-summary data. See `write_migration_upload_progress` in src/cli/migration.rs.
- **[migration] Upload action casing**: Match the GitHub identity prefix case-insensitively because the server returns configured casing, but keep the action origin, endpoint suffix, OID, and size exact. Diagnose rejected URLs through redacted expected and received forms. See `migration_upload_action_path_matches` in src/cli/migration.rs.
...
- **[storage] Drive setup transaction boundary**: Authorize and permission-check isolated ADC before writing a new storage entry; a successful `gcloud` exit without `application_default_credentials.json` is not a configured provider. See `authorize_google_drive_adc` in src/cli/configuration.rs.
- **[storage] Default Drive root identity**: Default setup uses `root` only as a request alias; Drive responses contain its canonical parent ID. Validate that concrete parent with the app marker, and persist the `.lfscloud` folder's real ID rather than its display name. See `GoogleDriveRootProvisioner` in src/google_drive/root.rs.
- **[storage] Drive reauthorization preservation**: Treat client-secret defaults as new-provider defaults only; reauthorization must use an existing provider's effective ADC directory without persisting omitted defaults over its custom root or credential settings. See `PreparedStorageProvider::with_effective_values` in src/cli/configuration.rs.
...
- **[tests] Pipe-reader scheduling**: A short-lived child can exit before loaded cross-architecture runners schedule its pipe-reader threads; allow a bounded post-exit scheduling grace before classifying incomplete readers as inherited pipes. See `OUTPUT_DRAIN_GRACE` in src/child_process.rs.
- **[tests] Descendant cleanup proof**: Unix `kill -0` also succeeds for terminated, unreaped zombies; prove pipe-holding descendant cleanup through bounded completion plus recovered output instead of treating PID existence as process liveness. See `successful_command_stops_pipe_holding_descendants` in src/credentials.rs.
...
- **[auth] Durable session key**: Production sessions persist only local-token hashes and AEAD-protected user GitHub PATs; prefer the stable native credential-store key, with `server.session_encryption_secret` reserved for explicit headless overrides. See `production_session_store` in src/server/runtime.rs.
