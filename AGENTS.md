...
- **[storage] Drive provider layers**: `GoogleDriveStorageProvider` supplies the shared contract; migration acquires its durable upload lock, while the server upload handler owns that lock across lookup and upload and composes generic metadata repair plus optional capabilities. See `handle_lfs_upload_request` in src/server.rs.
