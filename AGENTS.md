...
- **[migration] Dry-run readiness scope**: Dry runs report only local prerequisites; label TCP reachability, credential loading, and config mapping precisely because repository permission and Drive root probes would contact remote providers. See `migration_readiness_checks` in src/cli.rs.
