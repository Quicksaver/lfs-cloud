...
- **[build] Changelog release notes**: Version releases roll `Unreleased` into a dated version section before committing, and new or resumed GitHub drafts use that committed section as their description. See `release_roll_changelog` in scripts/lib/release-common.sh.
...
- **[release] Draft publication boundary**: Assemble macOS, Linux, Debian, installer, and Windows assets in an editable draft; only `release:publish` may publish it after all four trusted local statuses are green, while channel statuses make immutable-release distribution resumable. See `scripts/publish.ps1`.
