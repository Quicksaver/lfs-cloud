...
- **[cache] Verified ingest**: Repository-local Git LFS objects are hashed before and after temp-file publication into the shared cache, and existing cache files are reverified instead of overwritten. See `ingest_git_lfs_object` in src/local_cache.rs.
