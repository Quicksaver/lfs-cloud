...
- **[storage] Drive download gate**: Drive downloads must resolve file IDs through repository/OID/size app-property lookup before streaming `alt=media`; check media content length before proxying. See `GoogleDriveObjectStore::download_object_response` in src/google_drive.rs.
