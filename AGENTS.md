...
- **[tests] Storage contract parity**: Run the shared lifecycle contract against fakes, direct adapters, and migration wrappers. The fixture is also `include!`d by CLI unit tests, so keep its parent imports compatible and package `tests/` with crate sources. See `assert_storage_provider_contract` in tests/support/storage_provider_contract.rs.
...
- **[migration] Source fetch cleanup**: `git lfs fetch` may leave descendants holding pipes open, so setup failures and timeouts must stop the configured process tree; post-exit drains must preserve hard-limit errors before applying inherited-pipe policy. See `wait_for_child` in src/child_process.rs.
