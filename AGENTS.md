...
- **[cli] Init route base URL**: `init --server` rejects trailing-slash and raw whitespace/control/backslash server bases before appending the Git remote-derived LFS route, matching `server.public_url` behavior while preserving optional proxy base paths. See `LfsInitRoute` in src/init.rs.
